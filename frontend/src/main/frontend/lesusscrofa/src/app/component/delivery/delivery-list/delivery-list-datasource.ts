...
export class DeliveryListDatasource extends DataSource<DeliveryView> {
  private foodOrderClientFlatViewSubject = new BehaviorSubject<DeliveryView[]>([]);
  ...
  connect(): Observable<DeliveryView[]> {
  ...
  public loadData(foodOrderClientFlatView:DeliveryView[]) {
    this.foodOrderClientFlatViewSubject.next(foodOrderClientFlatView);
  }

  private getPagedData(data: DeliveryView[]) {
    const startIndex = this.paginator.pageIndex * this.paginator.pageSize;
    return data.slice(startIndex, this.paginator.pageSize);
  }

  private getSortedData(data: DeliveryView[]) {
